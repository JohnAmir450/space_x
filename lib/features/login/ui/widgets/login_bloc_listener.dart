...
class LoginBlocListener extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocListener<LoginCubit, LoginState>(
      listenWhen: (previous, current) => current is LoginCubitLoading||current is LoginCubitSuccess||current is LoginCubitFailure,
      listener: (context, state) {
       if(state is LoginCubitSuccess ){
        showSnackBar(context, text: 'Logged in Successfully', color: Colors.green);
